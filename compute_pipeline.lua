...
function ComputePipeline.Init(vk, device, pipelineLayout)
    ...
    return {
        ...
        pipelineLayout = pipelineLayout -- <--- STORE THIS SO WE CAN DESTROY IT!
    }
end
function ComputePipeline.Destroy(vk, core_state, comp_state)
    ...
    -- [THE FIX] Extract the raw C device pointer from the context table
    local device = core_state.device

    if comp_state.pipeline ~= nil then
        vk.vkDestroyPipeline(device, comp_state.pipeline, nil)
    end

    if comp_state.module ~= nil then
        vk.vkDestroyShaderModule(device, comp_state.module, nil)
    end
end
